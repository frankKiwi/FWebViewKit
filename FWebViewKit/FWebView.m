...
@interface FWebView () <UIWebViewDelegate,UIActionSheetDelegate,WKNavigationDelegate,UIScrollViewDelegate,WKScriptMessageHandler,WKUIDelegate>
...
/**
 加载模式
 */
@property (nonatomic, assign) FWebViewLoadType loadType;
...
- (BOOL)webView:(UIWebView *)webView shouldStartLoadWithRequest:(NSURLRequest *)request navigationType:(UIWebViewNavigationType)navigationType{
    ...
    if ([UIDevice currentDevice].systemVersion.doubleValue >= 9.0) {
        urlString = [NSString stringWithString:[urlString stringByRemovingPercentEncoding]];
    } else {
        urlString = [NSString stringWithString:[urlString stringByReplacingPercentEscapesUsingEncoding:NSUTF8StringEncoding]];
    }
    
     if ([urlString rangeOfString:@"自定义"].length != 0) {
